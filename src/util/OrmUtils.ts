export class OrmUtils {
  ...
  static mergeDeep(target: any, ...sources: any[]): any {
    ...
    if (this.isObject(target) && this.isObject(source)) {
      for (const key in source) {
        let propertyKey = key;
        if (source[key] instanceof Promise) continue;
        ...
        if (
          this.isObject(source[propertyKey]) &&
          ...
          if (!target[key])
            Object.assign(target, {
              [key]: Object.create(Object.getPrototypeOf(source[propertyKey]))
            });
  ...
  static deepCompare(...args: any[]) {
    ...
    for (i = 1, l = arguments.length; i < l; i++) {
      ...
      if (
        !this.compare2Objects(leftChain, rightChain, arguments[0], arguments[i])
      ) {
        return false;
      }
    }
